...
def score_to_lemmas(score):
    score_lemmas = []
    part = score.parts[0]
    for measure in part:
        score_lemmas = score_lemmas + ['bar']
        pos=0
        measure.show("text")
        for note in measure.flat.elements[1:]:
            pos+=1
            if isinstance(note, m21.chord.Chord):
                for a in note:
                    score_lemmas = score_lemmas + ['position_'+str(int((0.25+note.offset)*4))+'/16','Note_on('+a.nameWithOctave+')','Note_duration('+str(int(note.duration.quarterLength*4))+')','String('+str(a.articulations[0].number)+')','Fret('+str(a.articulations[1].number)+')']
            else:
                pass
    return np.reshape(np.array(score_lemmas),newshape=(len(score_lemmas),1))

def lemmas_to_onehot(score):
    ...
    # Encoding the score
    list_onehot = one_hot_encoder.transform(score_to_lemmas(score)).toarray()
